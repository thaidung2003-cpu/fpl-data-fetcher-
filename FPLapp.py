...
# We remove all pandas2ri conversion imports to completely avoid the contextvars threading bug!
...
def fetch_fpl_data():
    ...
    # 2. Native R Package Integration using File-Handoff (Bypasses ContextVars bug)
    try:
        setup_r_environment()
        
        # Execute the R logic natively and export to temporary local CSV files
        robjects.r('''
            library(panna)
            pb_download_opta()
        ...
        # Python reads the data generated by R, circumventing rpy2's broken memory converter
        opta_df = pd.read_csv("temp_stats.csv", low_memory=False)
        
        if os.path.exists("temp_xmetrics.csv"):
            xmetrics_df = pd.read_csv("temp_xmetrics.csv", low_memory=False)
        ...
        if os.path.exists("temp_stats.csv"):
            os.remove("temp_stats.csv")
        if os.path.exists("temp_xmetrics.csv"):
            os.remove("temp_xmetrics.csv")
